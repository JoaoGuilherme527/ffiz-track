import {formatDate, formatTime, formatUSDtoBRL} from "@/src/lib/utils"
...
export default function FormModalEditCard({
    ...
    const date = card ? formatDate(card.expirationDate) : ""
    return (
        <FormModal isModalOpen={isModalOpen} onClose={onClose} onSubmit={onSubmit} setIsModalOpen={setIsModalOpen}>
            ...
            <div className="flex flex-col gap-2 ">
                <label className="text-sm text-gray-700 flex place-items-baseline justify-between">Expiration date: {date}</label>
                <Input id="expirationDate" name="expirationDate" type="date" placeholder={formatDate(card?.expirationDate)} />
            </div>
            <div className="flex flex-col gap-2">
