...
const DashboardCard = ({src, title, value, description, variant}: DashboardCardProps) => (
    <div
        ...
        className={` relative p-4 border-[1px] border-gray-800  rounded-md flex flex-col  items-start justify-evenly flex-1 text-gray-700 shadow-sm gap-2 ${
            variant ? "bg-green-500" : "bg-gray-900"
        }`}
    >
        <div className={`rounded-full   ${variant ? "bg-white" : "bg-transparent"}`}>
            <Image src={src} alt="" width={50} height={50} className=" p-2 w-16 h-w-16" />
        </div>
        <h1 className={`font-bold text-sm   ${variant ? "text-gray-50" : "text-gray-200"}`}>{title}</h1>
        <p className={`font-extrabold text-base  ${variant ? "text-white" : "text-white"}`}>{value}</p>
    ...
    return (
        <>
            ...
            <div className="z-20 flex flex-col gap-4 p-4 w-full overflow-scroll md:hidden">
                ...
                <div className=" flex-2 flex flex-col p-4 border-[1px] border-gray-800 bg-gray-900 rounded-md gap-4 ">
                    <p className={`font-extrabold text-xl text-white `}>Recent Expenses</p>
                        ...
                        <div className="flex flex-col gap-4 w-full">
                            {expenses
                                ...
                                .map(({amount, name, transactionDate}: any, key: Key | null | undefined) => (
                                    <div key={key} className="relative  flex bg-gray-800 border-gray-950 py-4 px-2 gap-2 rounded">
                                        <div className="absolute top-1/2 translate-y-[-50%] left-[-8.4%] bg-green-400 rounded-full w-4 h-4"></div>
                                        <div className="flex flex-col w-full flex-1">
                                            <h1 className="text-lg text-white font-semibold break-words max-w-36">{name}</h1>
                                            <p className="text-xs text-white">
                                                {formatTime(transactionDate as string)}
                                            </p>
                                        </div>
                                        <p className={`text-base text-white font-bold truncate  cursor-pointer`}>{formatShortBRL(amount)}</p>
                                    </div>
                                ))}
                        </div>
                    </div>
